...
import com.xys.libzxing.zxing.camera.open.OpenCamera;
...
public final class CameraManager {
  ...
  private static final int MAX_FRAME_HEIGHT = 1280; // = 1/2 * 2560
  private static final int MAX_FRAME_WIDTH = 1080; // = 3/4 * 1440
  ...
  public synchronized Rect getFramingRect() {
    if (framingRect == null) {
      ...
      int width = findDesiredDimensionInRange(0,screenResolution.x, MIN_FRAME_WIDTH, MAX_FRAME_WIDTH);
      int height = findDesiredDimensionInRange(1,screenResolution.y, MIN_FRAME_HEIGHT, MAX_FRAME_HEIGHT);
  ...
  private static int findDesiredDimensionInRange(int direction,int resolution, int hardMin, int hardMax) {
    int dim = 0;
    if (direction==0){//宽度
      dim = 3 * resolution / 4; // Target 3/4 of each dimension
    }else{
      dim = 1 * resolution / 2; // Target 1/2 of each dimension
    }
    if (dim < hardMin) {
      return hardMin;
    }
