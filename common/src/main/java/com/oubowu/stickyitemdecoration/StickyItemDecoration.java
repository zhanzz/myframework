...
public class StickyItemDecoration extends RecyclerView.ItemDecoration {
    ...
    private OnStickyChangeListener mStickyListener;
    private boolean mEnableStickyHead = true;

    public StickyItemDecoration(OnStickyChangeListener stickyHeadContainer, int stickyHeadType) {
        mStickyListener = stickyHeadContainer;
    ...
    public void onDraw(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        if (mEnableStickyHead /*&& mFirstCompletelyVisiblePosition > mStickyHeadPosition*/ && mFirstVisiblePosition >= mStickyHeadPosition && mStickyHeadPosition != -1) {
            View belowView = parent.findChildViewUnder(c.getWidth() / 2, mStickyListener.getChildHeight() + 0.01f);
            mStickyListener.onDataChange(mStickyHeadPosition);
            int offset;
            if (isStickyHead(parent, belowView) && belowView.getTop() > 0) {
                offset = belowView.getTop() - mStickyListener.getChildHeight();
            } else {
                offset = 0;
            }
            mStickyListener.onScrollable(offset);
        } else {
            mStickyListener.onInVisible();
        }
    }
    ...
    public void enableStickyHead(boolean enableStickyHead) {
        ...
        if (!mEnableStickyHead) {
            mStickyListener.setVisibility(View.INVISIBLE);
        }
    }
